...
// ハウスマージン設定（エリア共通・ハウス種別の栽植設計のみ使用）
// { frameMarginM, entranceEdgeIndex, entranceWidthM, entranceDepthM, equipWidthM } | null
...
// ═══════════════════════════════════════════
//  🏠 ハウスマージン設定UI
//  （仕様書 セクション3。ridgeGeometry.js の computeHouseGeometry / getPolygonEdges /
//    calcRidges(...,holes) は既存実装済みのため、ここではUIと配線のみを行う）
// ═══════════════════════════════════════════

/**
 * ハウスマージン設定パネルのHTMLを生成する。
 * cultivationMode が greenhouse / heatedGreenhouse のエリアでのみ表示（露地は空文字）。
 */
function _adpBuildHouseMarginSection() {
  ...
  return `
    <div class="plt-housemargin-section">
      <div class="plt-housemargin-title">🏠 ハウスマージン設定</div>
      <div class="plt-housemargin-grid">
        ...
        <div class="plt-input-item">
          <label class="plt-label">入口幅</label>
          <div class="plt-input-wrap"><input type="number" class="plt-input" min="0" step="0.1" value="${hm.entranceWidthM ?? ''}" placeholder="例: 1.5"
            oninput="_adpUpdateHouseMarginField('entranceWidthM', this.value)"><span class="plt-unit">m</span></div>
        </div>
        <div class="plt-input-item">
        ...
        <div class="plt-input-item">
          <label class="plt-label">設備通路幅 <span class="plt-label-opt">任意</span></label>
          <div class="plt-input-wrap"><input type="number" class="plt-input" min="0" step="0.1" value="${hm.equipWidthM ?? ''}" placeholder="例: 0.8"
            oninput="_adpUpdateHouseMarginField('equipWidthM', this.value)"><span class="plt-unit">m</span></div>
        </div>
      </div>
      <div class="plt-housemargin-preview" data-housemargin-preview>
        ${_adpBuildHouseMarginPreviewInner()}
      </div>
    </div>`;
}

/**
 * ハウスマージンプレビュー部分（ミニSVG＋入口辺ステータス＋ローテートボタン）のHTML。
 * 入力欄の外側で完結させ、入力欄自体は再描画しない（フォーカスを失わないため）。
 */
function _adpBuildHouseMarginPreviewInner() {
  const polygon = _adpGetFieldPolygon();
  if (!polygon || typeof RidgeGeometry === 'undefined' || typeof RidgeGeometry.computeHouseGeometry !== 'function') {
    return '<div class="plt-housemargin-status">圃場データが取得できません</div>';
  }
  ...
  const svgHTML = _adpBuildEntranceEdgeMiniSVG(polygon, entranceIdx);
  ...
  return `
    ${svgHTML}
    <div class="plt-housemargin-status">${statusText}${areaText ? '／' + areaText : ''}</div>
...
/**
 * ハウスマージン入力欄の変更ハンドラ（外膜マージン・入口幅・入口奥行き・設備通路幅）。
...
/** ハウスマージンプレビュー部分（ミニSVG＋ステータス）だけを部分更新する。 */
function _adpRefreshHouseMarginSection() {
  const container = document.querySelector('#planting-result .plt-housemargin-preview');
  if (container) container.innerHTML = _adpBuildHouseMarginPreviewInner();
}
...
function _adpRenderPlantingPane() {
  ...
  if (_adpCurrentSeg === 'analysis') {
    ...
    if (!cropId) {
      el.innerHTML = _adpBuildAreaRidgeDirBar() + _adpBuildHouseMarginSection() + '<div class="empty-mini">作物を選択すると栽植設計の試算ができます。</div>';
      return;
    }
    const cropName = _adpCropIdToName(cropId);
  ...
  // ── 実務側 ──
  if (!_adpPracticecrops.length) {
    el.innerHTML = _adpBuildAreaRidgeDirBar() + _adpBuildHouseMarginSection() + '<div class="empty-mini">作物を追加すると栽植設計が入力できます。</div>';
    return;
  }
...
/**
 * 畝上幅・畝間の入力変更時に呼ぶ。実効ピッチ表示・畝セグメント・計算結果を更新する。
...
/**
 * 実務側専用：圃場の実形状に基づく畝プレビューSVGを生成する。
 * - 圃場ポリゴンの実外周線
 * - 全作物の帯（選択中＝緑、他作物＝薄グレー）を実位置・実形状で描画
 * - 選択中作物の畝（ridgeSegments の p1/p2 実座標）を実際の位置・向き・長さで描画
 * - ハウス（greenhouse/heatedGreenhouse）かつハウスマージン設定済みの場合：
 *   外膜マージンの実効境界（グレー破線）・入口ノッチ（オレンジ＋🚪ラベル）・
 *   設備通路ノッチ（青＋🔧ラベル）を重ねて表示し、下部に凡例を付ける
 * 座標変換は RidgeGeometry.toLocalCoords / toLocalPoint（重心原点のローカルXY）を
 * 一貫して使うことで、帯・畝・ハウスマージンすべてが同一座標系で正しく重なる。
 *
 * @param {string} cropId - プレビュー対象の作物ID（実務側の _adpPracticecrops 内）
 * @returns {string} HTML文字列
 */
function _adpBuildRidgeShapePreviewSVG(cropId) {
  ...
  if (!rawPolygon || typeof RidgeGeometry === 'undefined' || typeof RidgeGeometry.toLocalCoords !== 'function') {
    return '<div class="plt-shapesvg-empty">圃場データが取得できません</div>';
  }

  const { origin, points: fieldLocalPts } = RidgeGeometry.toLocalCoords(rawPolygon);
  if (!fieldLocalPts.length) {
    return '<div class="plt-shapesvg-empty">圃場データが取得できません</div>';
  }
  ...
  const PAD   = 34;  // 入口・設備通路ラベルがはみ出さないよう余裕を持たせる
  ...
  // --- 2. ハウスマージン（外膜オフセット境界・入口ノッチ・設備通路ノッチ） ---
  const isHouse = _adpArea?.cultivationMode === 'greenhouse' || _adpArea?.cultivationMode === 'heatedGreenhouse';
  const hm = _adpHouseMargin || null;
  let marginNoteHTML = '';
  let hasMarginLine = false, hasEntrance = false, hasEquip = false;
  let entranceWidthLabel = '', equipWidthLabel = '';

  if (isHouse && hm && typeof RidgeGeometry.computeHouseGeometry === 'function') {
    ...
    // computeHouseGeometry の穴の並び順（entrance→equip、それぞれ条件を満たす場合のみ）に合わせてラベルを対応させる
    const holeKinds = [];
    if (Number(hm.entranceWidthM) > 0 && Number(hm.entranceDepthM) > 0) holeKinds.push('entrance');
    if (Number(hm.equipWidthM) > 0) holeKinds.push('equip');
    ...
    (geo.holes || []).forEach((holeLatLngs, i) => {
      ...
      if (kind === 'entrance') {
        ...
        svgBody += `<text x="${labelSvg.x.toFixed(1)}" y="${labelSvg.y.toFixed(1)}" text-anchor="middle" class="plt-shapesvg-entrance-label">🚪入口 幅${hm.entranceWidthM}m</text>`;
        hasEntrance = true;
        entranceWidthLabel = `${hm.entranceWidthM}m`;
      } else if (kind === 'equip') {
        svgBody += `<polygon points="${ptsStr(holeSvgPts)}" class="plt-shapesvg-equip" />`;
        svgBody += `<text x="${labelSvg.x.toFixed(1)}" y="${labelSvg.y.toFixed(1)}" text-anchor="middle" class="plt-shapesvg-equip-label">🔧設備通路 幅${hm.equipWidthM}m</text>`;
        hasEquip = true;
        equipWidthLabel = `${hm.equipWidthM}m`;
      }
    });
  }

  // --- 3. 全作物の帯（選択中＝緑、他作物＝薄グレー背景） ---
  let otherCount = 0;
  (_adpPracticecrops || []).forEach(entry => {
    const band = entry?.plantingDesign?._bandPolygon;
    if (!Array.isArray(band) || band.length < 3) return;
    const bandSvgPts = band.map(p => toSvg(toLocal(p)));
    if (entry.cropId === cropId) {
      svgBody += `<polygon points="${ptsStr(bandSvgPts)}" class="plt-shapesvg-ownband" />`;
    } else {
      svgBody += `<polygon points="${ptsStr(bandSvgPts)}" class="plt-shapesvg-otherband" />`;
      otherCount++;
    }
  });
  ...
  // --- 5. 凡例（表示中の要素に応じて出し分け） ---
  const legendItems = [
    `<span class="plt-shapesvg-legend-item"><i class="plt-shapesvg-swatch plt-shapesvg-swatch-own"></i>選択中の作物の帯</span>`,
    `<span class="plt-shapesvg-legend-item"><i class="plt-shapesvg-swatch plt-shapesvg-swatch-ridge"></i>畝（線・実位置）</span>`,
  ];
  if (otherCount > 0) {
    legendItems.push(`<span class="plt-shapesvg-legend-item"><i class="plt-shapesvg-swatch plt-shapesvg-swatch-other"></i>他${otherCount}作物の帯</span>`);
  }
  if (hasMarginLine) {
  ...
  if (hasEntrance) {
    legendItems.push(`<span class="plt-shapesvg-legend-item"><i class="plt-shapesvg-swatch plt-shapesvg-swatch-entrance"></i>🚪 入口（幅${entranceWidthLabel}）</span>`);
  }
  if (hasEquip) {
